...
public class AssertUtil {
    ...
    public static <T> void fastFailValidate(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            throwException(ArgumentResponseEnum.PARAM_VALID, constraintViolations.iterator().next().getMessage());
        }
    }
    ...
    public static <T> void allCheckValidateThrow(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            ...
            //去掉最后一个逗号
            throwException(ArgumentResponseEnum.PARAM_VALID, errorMsg.toString().substring(0, errorMsg.length() - 1));
        }
    }
    ...
    //如果不是非空对象，则抛异常
    public static void isNotEmpty(Object obj, IResponseEnum errorEnum, Object... args) {
        if (isEmpty(obj)) {
            throwException(errorEnum, args);
        }
    }
    ...
    private static void throwException(IResponseEnum errorEnum, Object... arg) {
        if (Objects.isNull(errorEnum)) {
            errorEnum = CommonResponseEnum.BUSINESS_ERROR;
        }
        throw new BusinessException(errorEnum, new Object[]{arg}, MessageFormat.format(errorEnum.getMessage(), arg));
    }
