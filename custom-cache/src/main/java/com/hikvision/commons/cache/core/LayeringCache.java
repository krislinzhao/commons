package com.hikvision.commons.cache.core;

import com.hikvision.commons.cache.listener.RedisPubSubMessage;
import com.hikvision.commons.cache.listener.RedisPubSubMessageType;
import com.hikvision.commons.cache.listener.RedisPublisher;
...
import com.hikvision.commons.cache.stats.CacheStats;
import com.hikvision.commons.cache.support.CacheMode;
...
public class LayeringCache extends AbstractValueAdaptingCache {
     ...
     */
    public LayeringCache(RedisClient redisClient,
    ...
    public <T> T get(String key, Class<T> resultType) {
        ...
        if (!CacheMode.SECOND.equals(cacheMode)) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("查询一级缓存。 key={}:{},返回值是:{}", getName(), key, JsonUtil.toStr(result));
            }
            if (result != null) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("查询二级缓存,并将数据放到一级缓存。 key={}:{},返回值是:{}", getName(), key, JsonUtil.toStr(result));
        }
        return result;
    }
