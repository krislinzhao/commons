package com.hikvision.commons.cache.core.caffeine;

import com.hikvision.commons.cache.support.ExpireMode;
...
public class CaffeineCache extends AbstractValueAdaptingCache {
    ...
    public <T> T get(String key, Class<T> resultType) {
        if (logger.isDebugEnabled()) {
            logger.debug("caffeine缓存 key={} 获取缓存", JsonUtil.toStr(key));
        }
    ...
    public <T> T get(String key, Class<T> resultType, Callable<T> valueLoader) {
        if (logger.isDebugEnabled()) {
            logger.debug("caffeine缓存 key={} 获取缓存， 如果没有命中就走库加载缓存", JsonUtil.toStr(key));
        }
    ...
    public void put(String key, Object value) {
        ...
        if (isAllowNullValues()) {
            if (logger.isDebugEnabled()) {
                logger.debug("caffeine缓存 key={} put缓存，缓存值：{}", JsonUtil.toStr(key), JsonUtil.toStr(value));
            }
    ...
    public <T> T putIfAbsent(String key, Object value, Class<T> resultType) {
        if (logger.isDebugEnabled()) {
            logger.debug("caffeine缓存 key={} putIfAbsent 缓存，缓存值：{}", JsonUtil.toStr(key), JsonUtil.toStr(value));
        }
    ...
    public void evict(String key) {
        if (logger.isDebugEnabled()) {
            logger.debug("caffeine缓存 key={} 清除缓存", JsonUtil.toStr(key));
        }
    ...
    private <T> Object loaderValue(Object key, Callable<T> valueLoader) {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("caffeine缓存 key={} 从库加载缓存", JsonUtil.toStr(key), JsonUtil.toStr(t));
            }
