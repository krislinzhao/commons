package com.hikvision.commons.cache.core.redis;

import com.hikvision.commons.cache.support.AwaitThreadContainer;
import com.hikvision.commons.cache.support.LayeringCacheRedisLock;
import com.hikvision.commons.cache.support.NullValue;
import com.hikvision.commons.cache.util.ThreadTaskUtils;
import com.hikvision.commons.cache.core.AbstractValueAdaptingCache;
...
public class RedisCache extends AbstractValueAdaptingCache {
    ...
    public void put(String key, Object value) {
        RedisCacheKey redisCacheKey = getRedisCacheKey(key);
        if (logger.isDebugEnabled()) {
            logger.debug("redis缓存 key= {} put缓存，缓存值：{}", redisCacheKey.getKey(), JsonUtil.toStr(value));
        }
    ...
    public <T> T putIfAbsent(String key, Object value, Class<T> resultType) {
        if (logger.isDebugEnabled()) {
            logger.debug("redis缓存 key= {} putIfAbsent缓存，缓存值：{}", getRedisCacheKey(key).getKey(), JsonUtil.toStr(value));
        }
    ...
    private <T> T loaderAndPutValue(RedisCacheKey key, Callable<T> valueLoader, boolean isLoad) {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("redis缓存 key={} 执行被缓存的方法，并将其放入缓存, 耗时：{}。数据:{}", key.getKey(), System.currentTimeMillis() - start, JsonUtil.toStr(result));
            }
            ...
            try {
                if (redisLock.lock()) {
                    ...
                    if (isRefresh(redisCacheKey, preloadTime)) {
                        ...
                        if (!Objects.equals(oldDate, newDate) && !JsonUtil.toStr(oldDate).equals(JsonUtil.toStr(newDate))) {
